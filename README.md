...
이 맥에서 바로 설치해서 쓸 DMG는 아래 명령으로 만들 수 있습니다.
...
```text
dist/DdayPalette.dmg
```

DMG를 열고 `DdayPalette.app`을 `Applications` 폴더로 드래그한 뒤 앱을 한 번 실행하세요. 그 다음 바탕화면 우클릭 → 위젯 편집 → `Dday Palette`를 추가하면 됩니다.
